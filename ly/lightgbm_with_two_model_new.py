...
data['perdiction_is_null'] = data['query_prediction'].apply(lambda x: 1 if x == '{}' else 0)
...
train_val_data['prefix_len'] = train_val_data['prefix'].apply(lambda x: len(str(x)))
train_val_data['label'] = train_val_data['label'].apply(lambda x: int(x))
train_val_data["query_prediction"] = train_val_data["query_prediction"].apply(loads)
train_val_data['prediction_num'] = train_val_data['query_prediction'].map(lambda x: len(x.keys()))
...
for lr in [0.05]:
    ...
    for rand_seed in [100]:
        ...
        label_in = np.where(res_in > 0.39, 1, 0)
