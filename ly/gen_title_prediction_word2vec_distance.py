...
for i,row in data.iterrows():
    ...
    for item in s_list:
        if len(item) == 0:
            all_pre.append(0)
            all_distance.append(1)
            continue

        # item_list举例:[车辆行驶证, 0.018]
        item_list = [t.replace('"', '') for t in item.split('": "')]
        all_pre.append(float(item_list[-1]))
        all_distance.append(distance(sentence_vec(str(item_list[0]),title_is_all_zh), title_vec))
    ...
    if pre_mean > 0:
        title_max_prediction_distance = distance(sentence_vec(max_pre_title,title_is_all_zh), title_vec)

    title_is_all_zhs.append(title_is_all_zh)
    if not title_is_all_zh:
        title_tag_distances.append(1)

        title_prediction_distance_means.append(1)
        title_prediction_distance_stds.append(1)
        title_prediction_distance_mins.append(1)
        title_prediction_distance_maxs.append(1)
        title_prediction_distance_w_means.append(1)

        title_max_prediction_distances.append(1)
    else:
