...
model_path = '../Models/ly/11.12/'

train_data = pd.read_table(train_txt,names= ['prefix','query_prediction','title','tag','label'], header= None, encoding='utf-8',quoting=3).astype(str)
...
prefixs = pd.concat([train_data,val_data]).groupby(['prefix'], as_index=False)['prefix'].agg({'cnt': 'count'})['prefix'].tolist()
print('prefixs : ', len(prefixs))

# prefixs = pd.concat([train_data,val_data]).groupby(['prefix'], as_index=False)['prefix'].agg({'cnt': 'count'})

# print('before prefixs: ',len(prefixs))
# # ['prefix'].tolist()
# prefixs = prefixs[prefixs['cnt']>1]['prefix'].tolist()
# print('after prefixs : ', len(prefixs))
...
data['perdiction_is_null'] = data['query_prediction'].apply(lambda x: 1 if x == '' or x == '{}' else 0)
...
data['common_words_cnt'] = data.apply(lambda x: len(set(list(str(x['prefix']))).intersection(set(list(str(x['title']))))), axis=1)
...
train_val_data = shuffle(train_val_data)
...
for i in range(0, len_train_val, step):
    ...
    for i in range(len(items)):
        for j in range(i + 1, len(items)):
            item_g = [items[i], items[j]]
            temp = rest_data.groupby(item_g, as_index=False)['label'].agg({'_'.join(item_g) + '_click': 'sum', '_'.join(item_g) + '_count': 'count'})
            temp['_'.join(item_g) + '_ctr'] = temp['_'.join(item_g) + '_click'] / (
            temp['_'.join(item_g) + '_count'] + 3)
...
for item in items:
    ...
    mm = MinMaxScaler()
    test_data_ctr[[item + '_click', item + '_count']] = mm.fit_transform(test_data_ctr[[item + '_click', item + '_count']])
print('2 cross')
for i in range(len(items)):
    for j in range(i+1, len(items)):
        print(items[i],' ',items[j])
        item_g = [items[i], items[j]]
        temp = train_val_data.groupby(item_g, as_index=False)['label'].agg({'_'.join(item_g)+'_click': 'sum','_'.join(item_g)+'_count':'count'})
...
for curr_fold in range(nfolds):
    ...
    model = lgb.Booster(
        model_file=model_path + 'model_with_ctr' + str(curr_fold) + '.txt')
    # 利用模型对无标测试集做预测
    test_pred_in = model.predict(test_x_in)

    del model
    gc.collect()

    model2 = lgb.Booster(
        model_file=model_path + 'model_without_ctr' + str(curr_fold) + '.txt')
    test_pred_not_in = model2.predict(test_x_not_in)
...
submission['label'].to_csv(
        'result_online_threshold_' + str(online_threshold) + '_pre_with_model.csv', sep=',', index=False)
